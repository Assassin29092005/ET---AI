...
- `scheduler.py` — background asyncio task recomputing live corridor signals every `SCORE_REFRESH_SECONDS=300`, appending to `score_history` (pruned to 500/corridor), and fanning out to WebSocket subscribers when a score moves ≥ `SCORE_CHANGE_THRESHOLD=2.0` points or changes tier. `force_refresh()` triggers an immediate out-of-cycle recomputation.
